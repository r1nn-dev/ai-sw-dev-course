...
```
02_asgn/
├── ollama-namu-agent/     # 소스 코드
│   ├── results/           # 에이전트가 생성하는 결과 파일 저장 폴더
│   ├── main.py
│   ├── tools.py
│   ├── pyproject.toml
│   ├── README.md
│   └── .gitignore
├── guide.md               # 실습 계획 및 과정 정리 (현재 파일)
├── result.md              # 실행 결과 + 작동 원리 설명
└── images/                # 스크린샷 이미지
```
...
7. 추가 기능 — delete_file + results 폴더 관리
8. 동작 검증
9. `.gitignore` + GitHub 업로드
...
LLM: [TOOL_CALL] {"tool": "search_namuwiki", "args": {"keyword": "젤다"}}
    ↓
[도구 호출] search_namuwiki(keyword=젤다)
[도구 결과] 젤다의 전설은 닌텐도가 개발한...
    ↓
히스토리에 추가 → LLM 재호출
    ↓
LLM: [TOOL_CALL] {"tool": "write_file", "args": {"path": "zelda.md", "content": "..."}}
    ↓
[도구 호출] write_file(path=zelda.md)
[도구 결과] 파일이 저장되었습니다.
    ↓
히스토리에 추가 → LLM 재호출
    ↓
...
### 7. 추가 기능 — delete_file + results/ 폴더 관리
- `delete_file` : 파일 삭제 기능 함수
- `results/` : 폴더 자동 관리. 파일명만 입력해도 자동으로 results/ 폴더에 저장.
...
```
.venv는 git에 안 올려도 되니,
마찬가지로 올리지 않아도 되는 것들을 찾아서 .gitignore 파일을 만들어줘.
```
